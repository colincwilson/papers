...
class Config:
    ...
    def __init__(self,
        ...
        self.editor = editor
        self.bibtex = bibtex
        self.keyformat = copy.deepcopy(
            keyformat if keyformat is not None else KEYFORMAT)
        self.nameformat = copy.deepcopy(
            nameformat if nameformat is not None else NAMEFORMAT)
    ...
    def _relpath(self, p):
        ...
        try:
            ...
            print(error)
            logger.warning(
                f"config :: can't save {p} as relative path to {self.root}")
            return p
    ...
    def _update_paths_to_absolute(self):
        if self.file is None:
            logger.warning(
                "_update_paths_to_absolute: only works if Config.file is defined"
            )
            return
    ...
    def status(self, check_files=False, verbose=False):
        ...
        lines.append(bcolors.BOLD + f'papers configuration {status}' +
                     bcolors.ENDC)
        lines.append(bcolors.BOLD + f'version {__version__}' + bcolors.ENDC)
        if verbose:
            lines.append('* configuration file: ' +
                         (_fmt_path(self.file) if self.file
                          and os.path.exists(self.file) else bcolors.WARNING +
                          'none' + bcolors.ENDC))
        ...
        if self.filesdir is None:
            ...
            status = bcolors.OKBLUE + " ({} files, {:.1f} MB)".format(
                file_count, folder_size / (1024 * 1024.0)) + bcolors.ENDC
        else:
            status = ''

        lines.append(
            f'* files directory:    {_fmt_path(self.filesdir) if self.filesdir else self.filesdir}'
            + status)
        ...
        if self.bibtex is None:
            ...
            try:
                ...
                if len(db.entries):
                    status = bcolors.OKBLUE + ' ({} entries)'.format(
                        len(db.entries)) + bcolors.ENDC
                else:
                    status = bcolors.WARNING + ' (empty)' + bcolors.ENDC
            except:
            ...
            status = ''
        lines.append(
            f'* bibtex:             {_fmt_path(self.bibtex) if self.bibtex else self.bibtex}'
            + status)
